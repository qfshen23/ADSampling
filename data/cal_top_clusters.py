...
def process_batch(args):
    batch, centroids, num_clusters = args
    # Compute distances to all centroids
    distances = faiss.pairwise_distances(batch, centroids)
    # Sort cluster IDs by distance
    top_clusters = np.argsort(distances, axis=1)
    
    # Convert to bytes
    result = bytearray()
    for cluster_ids in top_clusters:
        result.extend(struct.pack('i', num_clusters))  # Write dimension
        result.extend(struct.pack(f'{num_clusters}i', *cluster_ids.astype(np.int32)))
    return result
...
def compute_and_save_top_clusters(X, centroids_path, output_path, batch_size=10000):
    ...
    for i in range(0, X.shape[0], batch_size):
        ...
        batches.append((X[i:end], centroids, num_clusters))
...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        output_path = os.path.join(path, f'{dataset}_top_clusters_{K}.ivecs')
