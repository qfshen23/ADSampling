...
def compute_weighted_overlap(query_clusters, base_clusters, k, query2centroids_dist):
    """
    针对 base vector 的 top-k clusters，如果其中一个 cluster 也属于 query 的 top-k clusters，
    则加上 1 / dis(query, this cluster centroid)
    """
    score = 0.0
    query_clusters_set = set(query_clusters[:k])
    for c in base_clusters[:k]:
        if c in query_clusters_set:
            dist = query2centroids_dist[c]
            score += 1.0 / (dist + 1e-6)  # 防止除零
    return score
...
def main():
    ...
    for dataset in datasets:
        ...
        for query_idx in tqdm(range(min(1000, num_queries))):
            ...
            distances = np.sum((query - centroids) ** 2, axis=1)  # [num_centroids]
            ...
            query_top_clusters = np.argsort(distances)  # cluster 索引按距离升序
            query2centroids_dist = distances  # query 到所有 cluster 的距离

            overlap_scores = []
            for vector_id in probe_vector_ids:
                vector_top_clusters = top_clusters[vector_id]
                score = compute_weighted_overlap(query_top_clusters, vector_top_clusters, k_overlap, query2centroids_dist)
